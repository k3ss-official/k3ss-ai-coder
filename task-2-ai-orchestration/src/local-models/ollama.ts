...
export class OllamaProvider extends BaseAIProvider {
  ...
  async loadModels(): Promise<void> {
    try {
      ...
      this.models = data.models?.map((model: OllamaModel) => this.mapOllamaModel(model)) || [];
    } catch (error) {
  ...
  private mapOllamaModel(ollamaModel: OllamaModel): AIModel {
  ...
  private getModelAccuracy(modelName: string): number {
    ...
    if (modelName.includes('70b')) return 0.92;
    if (modelName.includes('34b')) return 0.88;
    if (modelName.includes('13b')) return 0.85;
    if (modelName.includes('7b')) return 0.82;
  ...
  private buildContextContent(context: any): string {
    ...
    if (context.files && context.files.length > 0) {
      content += '\nRelevant files:\n';
      context.files.slice(0, 2).forEach((file: any, index: number) => {
        content += `\n${index + 1}. ${file.path}:\n\`\`\`${file.language || ''}\n`;
