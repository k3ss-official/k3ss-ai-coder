...
class LMStudioProvider extends base_1.BaseAIProvider {
    constructor(config) {
    ...
    async loadModels() {
        try {
            ...
            this.models = data.data?.map((model) => this.mapLMStudioModel(model)) || [];
        }
        catch (error) {
    ...
    async sendRequest(request) {
        ...
        try {
            ...
            throw this.createError('REQUEST_FAILED', `LM Studio request failed: ${error.message}`, true);
        }
    }
    getCapabilities() {
        return {
    ...
    mapLMStudioModel(lmStudioModel) {
    ...
    buildContextContent(context) {
        ...
        if (context.dependencies && context.dependencies.length > 0) {
            ...
            context.dependencies.slice(0, 8).forEach((dep) => {
